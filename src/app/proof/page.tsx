...
export default async function ProofPage() {
  ...
  return <section className="page-stack proof-page">
    ...
    <section className="proof-evidence-grid">
      ...
      <article className="surface"><h3>最近运行 checkpoint</h3>{latestRun ? <><p>{latestRun.executionMode === "live" ? "实时运行" : latestRun.executionMode === "replay" ? "真实历史运行回放" : latestRun.executionMode === "demo" ? "演示运行" : "旧记录（模式未标注）"} · {latestRun.runStage ?? latestRun.stage}</p><p>重试 {latestRun.retryCount ?? 0} 次 · 心跳 {latestRun.heartbeatAt ? new Date(latestRun.heartbeatAt).toLocaleString("zh-CN") : "未记录"}</p>{latestRun.checkpoints?.length ? <ol className="checkpoint-list">{latestRun.checkpoints.map((checkpoint, index) => <li key={`${checkpoint.stage}-${checkpoint.startedAt}-${index}`}><strong>{checkpoint.stage}</strong><small>{checkpoint.executionMode} · {new Date(checkpoint.startedAt).toLocaleTimeString("zh-CN")}{checkpoint.completedAt ? " → 已结束" : " → 进行中"}</small></li>)}</ol> : <small>旧运行没有阶段历史。</small>}</> : <p>尚无运行账本</p>}</article>
    </section>
