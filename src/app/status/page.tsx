...
import { DAILY_CANDIDATE_LIMIT, DAILY_PRIORITY_LIMIT } from "@/core/creator-desk";
...
export default async function StatusPage() {
  ...
  return <section className="page-stack">
    ...
    <section className="surface status-overview"><div><span>自动运行</span><strong>{scheduler.state === "enabled" ? "已启用" : "未启用"}</strong></div><div><span>核心 Mind</span><strong>{dashboard.systemStatus.mind.state === "connected" ? dashboard.systemStatus.mind.mindName : "未连接"}</strong></div><div><span>契约版本</span><strong>{run?.contractVersion ?? CREATOR_AGENT_CONTRACT.version}</strong></div><div><span>下一次</span><strong>{scheduler.state === "enabled" ? formatTime(scheduler.nextRunAt) : "—"}</strong></div></section>
    {scheduler.state === "enabled" ? <section className="surface status-detail"><h3>Mind-first 自动任务</h3><p>定时器每天采集候选；Mind 负责筛选、排序并决定生成或跳过。</p><dl><dt>目标平台</dt><dd>{scheduler.platform === "x" ? "X" : "小红书"}</dd><dt>每日筛选</dt><dd>{DAILY_CANDIDATE_LIMIT} 条候选，优先保留 {DAILY_PRIORITY_LIMIT} 条</dd><dt>最多输出</dt><dd>{scheduler.outputCount ?? 1} 条</dd><dt>最近结果</dt><dd>{scheduler.lastOutcome === "skipped" ? `已筛选 ${scheduler.lastCandidateCount ?? 0} 条、保留 ${scheduler.lastPriorityCount ?? 0} 条优先项，Mind 跳过本轮` : scheduler.lastOutcome === "drafted" ? `已筛选 ${scheduler.lastCandidateCount ?? 0} 条、保留 ${scheduler.lastPriorityCount ?? 0} 条优先项并送入今日内容` : "尚无"}</dd>{scheduler.lastPlan ? <><dt>最近决策</dt><dd>{scheduler.lastPlan.reason}</dd><dt>记忆影响</dt><dd>{scheduler.lastPlan.memoryInfluence}</dd></> : null}</dl>{scheduler.lastError ? <p className="settings-error">{scheduler.lastError}</p> : null}</section> : <section className="surface empty-state">前往设置连接 Mind 并启用自动运行。<br /><Link href="/settings/connections">打开设置</Link></section>}
    ...
    {run?.gateResults?.length ? <section className="surface status-detail"><h3>验收门</h3><dl>{run.gateResults.map((result) => <div key={result.gate}><dt>{AGENT_GATE_LABELS[result.gate]}</dt><dd>{result.status === "passed" ? "通过" : result.status === "rejected" ? "未通过" : "待确认"} · {result.detail}</dd></div>)}</dl></section> : null}
  </section>;
}
